...
export const DEFAULT_CENTER: UserLocation = { latitude: 40.7128, longitude: -74.006, label: "Lower Manhattan fallback" };

export function getMarkerColor(record: Partial<Pothole>): string {
  if (record.status === "closed") return "#5fd8a7";
  const risk = record.risk_score ?? 0;
  if (risk >= 80) return "#ff6b57";
  if (risk >= 55) return "#ffb347";
  return "#74e6c3";
}

export function getRiskColor(score?: number | null): string {
  if ((score ?? 0) >= 80) return "#ff6b57";
  if ((score ?? 0) >= 55) return "#ffb347";
  return "#74e6c3";
}

export function getUrgencyLabel(tier?: number | null): string {
  return ({ 0: "Low", 1: "Moderate", 2: "High", 3: "Critical" }[tier ?? -1]) || "Unknown";
}
...
export function getDistanceMiles(from: Pick<UserLocation, "latitude" | "longitude">, to: Pick<Pothole, "latitude" | "longitude">): number {
  const R = 3958.8;
...
export function withinBounds(pothole: Pothole, bounds?: { getSouth(): number; getNorth(): number; getWest(): number; getEast(): number } | null): boolean {
  ...
  return pothole.latitude >= bounds.getSouth() && pothole.latitude <= bounds.getNorth() && pothole.longitude >= bounds.getWest() && pothole.longitude <= bounds.getEast();
}

export function matchesFilters(pothole: Pothole, filters: PotholeFilters): boolean {
  if (filters.borough && pothole.borough !== filters.borough) return false;
  if (filters.status && pothole.status !== filters.status) return false;
  return true;
}

export function getLocationLabel(location: UserLocation | null, status: string): string {
  if (location) return location.label;
  if (status === "locating") return "Requesting your location";
  if (status === "denied") return "Location denied, using NYC fallback";
