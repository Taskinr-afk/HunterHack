...
def _parquet_cache_supported() -> bool:
    try:
...
PARQUET_CACHE_SUPPORTED = _parquet_cache_supported()


def _pickle_cache_path(path: Path) -> Path:
    return path.with_suffix(".pkl")


def _read_cache(path: Path) -> pd.DataFrame | None:
    if PARQUET_CACHE_SUPPORTED and path.exists():
        return pd.read_parquet(path)

    pickle_path = _pickle_cache_path(path)
    if pickle_path.exists():
        return pd.read_pickle(pickle_path)

    if path.exists() and not PARQUET_CACHE_SUPPORTED:
        print(f"  [data] Skipping {path.name}; parquet engine unavailable.")
...
def _write_cache(df: pd.DataFrame, path: Path) -> None:
    ...
    if PARQUET_CACHE_SUPPORTED:
        df.to_parquet(path, index=False)
        return
    df.to_pickle(_pickle_cache_path(path))
...
def fetch_all(pothole_limit: int = 50_000, use_cache: bool = True) -> pd.DataFrame:
    ...
    Filters 311 data to last 5 years for deeper ML training.
    Added columns: traffic_volume, aadt, nearby_crashes, pavement_crash_nearby.
    Set use_cache=False to force re-fetch even when cache files exist.
    """
    if use_cache:
        cached = _read_cache(ENRICHED_CACHE)
...
def _fetch_potholes(limit: int = 50_000) -> pd.DataFrame:
    ...
    params = {
        ...
        "$select": (
            ...
            "complaint_type,descriptor,borough,street_name,incident_address,"
...
def _fetch_traffic(limit: int = 200_000, use_cache: bool = True) -> pd.DataFrame:
    ...
    daily = raw.groupby(["segmentid", "street", "boro", "date"])["vol"].sum().reset_index()
    seg_avg = (
    ...
    _write_cache(seg_avg, TRAFFIC_CACHE)
    return seg_avg
...
def _fetch_collisions(limit: int = 100_000, use_cache: bool = True) -> pd.DataFrame:
    ...
    _write_cache(df, COLLISION_CACHE)
    return df
...
def _join_traffic(potholes: pd.DataFrame, traffic: pd.DataFrame) -> pd.DataFrame:
    ...
    lookup = traffic.groupby(["street_key", "boro_key"])["daily_avg_vol"].max().to_dict()
    ...
    df["traffic_volume"] = [
        lookup.get((street_key, borough_key), boro_median.get(borough_key, np.nan))
        for street_key, borough_key in zip(sk, bk)
    ]
    return df
...
def _join_aadt(potholes: pd.DataFrame, aadt: pd.DataFrame) -> pd.DataFrame:
    ...
    lookup = aadt.groupby(["street_key", "boro_key"])["count"].max().to_dict()
...
def _join_collisions(potholes: pd.DataFrame, collisions: pd.DataFrame) -> pd.DataFrame:
    ...
    df["nearby_crashes"] = tree.query_radius(
        pot_coords, r=CRASH_RADIUS_M / earth_radius_m, count_only=True
    )
    ...
    if pav_mask.any():
        ...
        df["pavement_crash_nearby"] = (
            p_tree.query_radius(
                pot_coords,
                r=PAVEMENT_CRASH_RADIUS_M / earth_radius_m,
                count_only=True,
            )
            > 0
        ).astype(int)
...
def _clean_text_series(series: pd.Series, default: str = "") -> pd.Series:
    cleaned = series.where(series.notna(), default).astype(str).str.strip()
...
def _clean_potholes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    street_name = df["street_name"] if "street_name" in df.columns else pd.Series("", index=df.index)
    incident_address = (
        df["incident_address"] if "incident_address" in df.columns else pd.Series("", index=df.index)
    )
    street_name = _clean_text_series(street_name, "")
    incident_address = _clean_text_series(incident_address, "")
    df["street_name"] = street_name.mask(street_name.eq(""), incident_address)
    ...
    for col, default in [
        ...
        df[col] = _clean_text_series(col_data, default)
...
def _norm_street(name: str) -> str:
    if not isinstance(name, str):
        return ""
    cleaned = _ABBREV.sub("", name.upper().strip())
...
def _print_data_quality(df: pd.DataFrame) -> None:
    ...
    pavement_pct = (
        df["pavement_crash_nearby"].mean() * 100 if "pavement_crash_nearby" in df.columns else 0
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-cache", action="store_true", help="Force re-fetch and ignore cached files")
